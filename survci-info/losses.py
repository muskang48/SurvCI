...
def mse_loss(model,x,t,c,w):
  ...
  c_pred_co = auc(model,c[w==0],shape_co_c,scale_co_c,logits_co_c)
  c_pred_tr = auc(model,c[w==1], shape_tr_c, scale_tr_c,logits_tr_c)
  #pdb.set_trace()
...
def _conditional_lognormal_loss(t,c, e, shape,scale,logits,shape_c,scale_c,logits_c,alpha, elbo=True):
...
def conditional_loss(model, x, t,c, e, w, elbo=True):
  ...
  if model.dist == 'Weibull':
    ...
    cond_loss_co =  _conditional_lognormal_loss(t[w==0], c[w==0],e[w==0],shape_co,scale_co,logits_co,shape_co_c, scale_co_c, logits_co_c,alpha, elbo)
    cond_loss_tr = _conditional_lognormal_loss(t[w==1], c[w==1], e[w==1],shape_tr,scale_tr,logits_tr, shape_tr_c, scale_tr_c, logits_tr_c,alpha,elbo)
  else:
    raise NotImplementedError('Distribution: '+model.dist+
                              ' not implemented yet.')
