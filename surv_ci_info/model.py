...
class survci_info(nn.Module):

  def _init_survci_info_layers(self, lastdim):
    ...
    self._init_survci_info_layers_cens(lastdim)

  def _init_survci_info_layers_cens(self, lastdim):

    if self.dist in ['Weibull']:
      self.act = nn.SELU()
      self.shape_c = nn.ParameterDict({str(r+1): nn.Parameter(-torch.ones(self.k))
                                     for r in range(self.num_treatments)})
      self.scale_c = nn.ParameterDict({str(r+1): nn.Parameter(-torch.ones(self.k))
                                     for r in range(self.num_treatments)})
    elif self.dist in ['Normal']:
      self.act = nn.Identity()
      self.shape_c = nn.ParameterDict({str(r+1): nn.Parameter(torch.ones(self.k))
    ...
    self.gate_c = nn.ModuleDict({str(r+1): nn.Sequential(
        nn.Linear(lastdim, self.k, bias=False)
        ) for r in range(self.num_treatments)})

    self.scaleg_c = nn.ModuleDict({str(r+1): nn.Sequential(
        nn.Linear(lastdim, self.k, bias=True)
        ) for r in range(self.num_treatments)})

    self.shapeg_c = nn.ModuleDict({str(r+1): nn.Sequential(
        nn.Linear(lastdim, self.k, bias=True)
        ) for r in range(self.num_treatments)})
  ...
  def __init__(self, inputdim, k, layers=None, dist='LogNormal',
    ...
    self._init_survci_info_layers(lastdim)
  ...
  def forward(self,x,W):
    ...
    beta_co_c = torch.ones((xrep[W==0].shape[0], self.k),dtype=torch.float64) #change
    eta_co_c = torch.ones((xrep[W==0].shape[0], self.k),dtype=torch.float64) #change
    logits_co_c = torch.ones((xrep[W==0].shape[0], self.k),dtype=torch.float64) #change
    beta_tr_c = torch.ones((xrep[W==1].shape[0], self.k),dtype=torch.float64) #change
    eta_tr_c = torch.ones((xrep[W==1].shape[0], self.k),dtype=torch.float64) #change
    logits_tr_c = torch.ones((xrep[W==1].shape[0], self.k),dtype=torch.float64) #change
    ...
    eta_co_c,beta_co_c,logits_co_c =  self.act(self.shapeg_c[str(1)](xrep_co))+self.shape_c[str(1)].expand(dim0, -1),self.act(self.scaleg_c[str(1)](xrep_co))+self.scale_c[str(1)].expand(dim0, -1),self.gate_c[str(1)](xrep_co)/self.temp
  ...
  def get_shape_scale_c(self,W):
    num_tr = int(W.sum())
    num_co = len(W) - num_tr
    shape_co_c = torch.ones((num_co,self.k), dtype=torch.float64) #change
    scale_co_c = torch.ones((num_co,self.k),dtype=torch.float64) #change
    shape_tr_c = torch.ones((num_tr,self.k), dtype=torch.float64) #change
    scale_tr_c = torch.ones((num_tr,self.k),dtype=torch.float64) #change
    shape_co_c = self.shape_c[str(1)]
    scale_co_c = self.scale_c[str(1)]
    shape_tr_c = self.shape_c[str(2)]
    scale_tr_c = self.scale_c[str(2)]
    return(shape_co_c,scale_co_c,shape_tr_c,scale_tr_c)
