...
# from csa.losses import conditional_loss,unconditional_loss,mse_loss,imb_loss
# from csa.model import CSA
...
def get_parameters(model,x,w):
  ...
  return shape_co, scale_co, logits_co, shape_tr, scale_tr, logits_tr,shape_co_c, scale_co_c, logits_co_c, shape_tr_c, scale_tr_c, logits_tr_c
...
def auc(model, t, shape, scale, logits):
  ...
  T = torch.tensor(np.arange(0.001,torch.max(t.cpu()),1)).clone().detach()
  #T = torch.tensor(np.arange(0.01,torch.max(t),0.01)).clone().detach()
  surv_auc = []
  for i in range(shape.shape[0]):
    shape_temp = shape[i].repeat(len(T))
