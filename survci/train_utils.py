...
from survci.losses import conditional_loss,unconditional_loss,mse_loss,mse_total,imb_loss,factual_loss,unconditional_mse,l2_loss
from survci.utilities import get_optimizer
...
def pretrain(model, t_train, e_train, W_train, t_valid, e_valid,W_valid,
                 n_iter=10000, lr=1e-2, thres=1e-4):
  '''Pretrain Model to get Prior Parameters for the Distribution'''

  
  premodel = survci(1, 1,dist=model.dist,num_treatments=model.num_treatments,optimizer=model.optimizer)
  ...
  for i in tqdm(range(n_iter)):
    ...
    loss = unconditional_loss(premodel, t_train.cuda(), e_train.cuda(), W_train.cuda())
    ...
    valid_loss = unconditional_loss(premodel, t_valid.cuda(), e_valid.cuda(), W_valid.cuda())
...
def train_survci(model,
              x_train, t_train, e_train,W_train,
              x_valid, t_valid, e_valid,W_valid,name,
...
#lr =1e-2
  ...
  valid_total_loss = []
  i = 0
  writer = SummaryWriter(f'runs/p_alpha{model.p_alpha}p_beta{model.p_beta}p_lamda{model.p_lamda}_{name}')
  for i in tqdm(range(n_iter)):
    epoch_ll_loss = 0
    epoch_ipm_loss = 0
    epoch_mse_loss = 0
    ...
    for j in range(nbatches):
      ...
      loss_ll = conditional_loss(model,xb,_reshape_tensor_with_nans(tb),_reshape_tensor_with_nans(eb),_reshape_tensor_with_nans(wb),elbo=elbo)
      ipm_loss = imb_loss(model,xb,_reshape_tensor_with_nans(wb))
      # pdb.set_trace()
      mse = mse_loss(model,xb,_reshape_tensor_with_nans(tb),_reshape_tensor_with_nans(wb))
      l_f = factual_loss(model,xb,_reshape_tensor_with_nans(tb),_reshape_tensor_with_nans(eb),_reshape_tensor_with_nans(wb))
      l2 = l2_loss(model)
      #mse = mse_total(model,xb,_reshape_tensor_with_nans(tb),_reshape_tensor_with_nans(eb),_reshape_tensor_with_nans(wb))
      loss = loss_ll + model.p_alpha*ipm_loss + model.p_beta*mse + model.p_lamda*l2
    ...
    writer.add_scalar("train/Loss_mse", epoch_mse_loss/nbatches, i)
    writer.add_scalar("train/Loss_l2", epoch_l2_loss/nbatches, i)
    
    
    model1 = model.eval()
    valid_ll = conditional_loss(model1,x_valid,t_valid_,e_valid_,W_valid_,elbo=False)
    valid_ipm = imb_loss(model1,x_valid,W_valid_)
    # pdb.set_trace()
    valid_mse = mse_loss(model1,x_valid,t_valid_,W_valid_)
    ...
    valid_loss = valid_ll + model1.p_alpha*valid_ipm + model1.p_beta*valid_mse + model1.p_lamda*valid_l2
    ...
    writer.add_scalar("valid/Loss_l2",valid_l2,i)
    
    print('\n--------------------------------------------------')
    print('Epoch: {}  Train Total Loss: {:.4f} Train elbo Loss: {:.4f}  Train ipm Loss: {:.4f} Train mse Loss: {:.4f} Train l2 loss {:.4f} '.format(i, loss, loss_ll, ipm_loss, mse,l2))
    print('--------------------------------------------------')
    print('\n--------------------------------------------------')
    print('Epoch: {}  Val Total Loss: {:.4f}  Val elbo Loss: {:.4f} Val ipm Loss: {:.4f} Val mse Loss: {:.4f} Valid l2 loss {:.4f} '.format(i, valid_loss,valid_ll, valid_ipm,valid_mse,valid_l2))
    print('--------------------------------------------------')
    valid_loss = valid_loss.detach().cpu().numpy()
    ...
    if costs[-1] >= oldcost:
      if patience == 2:
        ...
        del dics
        gc.collect()
        torch.save(model.state_dict(),f"survci_p-alpha{model.p_alpha}_p-beta{model.p_beta}_{model.p_lamda}_{name}.pth")
        writer.add_hparams({'Lr':lr ,'p_alpha':model.p_alpha,'p_beta':model.p_beta,'p_lamda':model.p_lamda},{'train_total_loss':np.mean(train_total_loss),'train_ll_loss':np.mean(train_ll_loss),'train_ipm_loss':np.mean(train_ipm_loss),'train_mse_loss':np.mean(train_mse_loss)})
        return model, i
      else:
        patience += 1
    else:
  ...
  torch.save(model.state_dict(),f"survci_p-alpha{model.p_alpha}_p-beta{model.p_beta}_{model.p_lamda}_{name}.pth")
  writer.add_hparams({'Lr':lr ,'p_alpha':model.p_alpha,'p_beta':model.p_beta,'p_lamda':model.p_lamda},{'train_total_loss':np.mean(train_total_loss),'train_ll_loss':np.mean(train_ll_loss),'train_ipm_loss':np.mean(train_ipm_loss),'train_mse_loss':np.mean(train_mse_loss)})
  return model, i #, np.mean(train_total_loss),np.mean(valid_total_loss)
