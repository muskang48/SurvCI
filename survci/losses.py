...
from surv_ci.utilities import lindisc, mmd2_lin, auc, auc_unconditional
...
def factual_loss(model, x,t,e,w):
  ...
  return  torch.mean(e*torch.abs(pred_t-true_t) + (1-e)*relu(pred_t-true_t))
...
def conditional_loss(model, x, t, e, w, elbo=True):
  ...
  if model.dist == 'Weibull':
    ...
    cond_loss_co =  _conditional_lognormal_loss(t[w==0], e[w==0],shape_co,scale_co,logits_co,alpha, elbo)
    cond_loss_tr = _conditional_lognormal_loss(t[w==1], e[w==1],shape_tr,scale_tr,logits_tr, alpha,elbo)
  else:
    raise NotImplementedError('Distribution: '+model.dist+
                              ' not implemented yet.')
